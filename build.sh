...
./configure --prefix=$target --enable-ipv6 --enable-unicode=ucs4 \
  --enable-optimizations --with-lto --with-computed-gotos --with-fpectl \
  CC=$CC CXX=$CXX CFLAGS="-g" LDFLAGS="-g -Xlinker -export-dynamic -rdynamic -Bsymbolic-functions -Wl,-z,relro" LIBS="-lffi -lbz2 -luuid -lsqlite3 -llzma"
...
sudo mv $target/lib/python3.9/pip.py $target/lib/python3.9/pip.py.bak && sudo $target/bin/python -m ensurepip && sudo mv $target/lib/python3.9/pip.py.bak $target/lib/python3.9/pip.py
